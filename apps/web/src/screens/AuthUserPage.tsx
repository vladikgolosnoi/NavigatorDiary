...
export function AuthUserPage() {
  ...
  const submitPasswordResetRequest = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setResetNotice(
        'Заявка на восстановление отправлена. Организатор задаст временный пароль. Если для вашего аккаунта настроена рабочая почта, пароль придет туда, иначе его передадут вручную.'
      )
            ...
            {showResetForm ? (
              <form className="form-card" onSubmit={submitPasswordResetRequest}>
                ...
                <p className="hint">
                  ...
                  пароль. Если для аккаунта есть рабочая почта и подключена отправка писем, пароль придет на email.
                  Иначе организатор передаст его вручную.
                </p>
